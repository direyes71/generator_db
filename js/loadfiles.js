$(document).ready(function(){
	...
	function loadFile() {
  	...
  	function load_connections(source, target){
  		// setup some defaults for jsPlumb.

  		var var_init =  ini_library();
  		var instance = var_init.instance;
  		var windows = var_init.windows;
  	...
  	$('.add_element').click(function(evt){
		...
		if (label != null) {
			...
			/*
			jsPlumb.draggable(id_target);

			var endpointOptions = { isTarget:true, endpoint:"Rectangle", paintStyle:{ fillStyle:"gray" } };
			var endpoint = jsPlumb.addEndpoint(id_target, endpointOptions);
			*/
		    ...
		    */
			...
			//var e1 = jsPlumb.addEndpoint(id_target, endpoint);

			//jsPlumb.addEndpoint("statemachine-demo", {uuid:id_target}); 
  		}
  	});
